...
import { NOTES, SCALES, getRandomItem, DIFFICULTY_PRESETS } from '../utils/musicTheory';
...
export function ScaleTraining({ gameMode, difficulty, settings }: Props) {
  ...
  const generateNewQuestion = () => {
    ...
    const scaleName = getRandomItem(preset.scales);
  ...
  const playSound = () => {
    if (currentScale) {
      audioEngine.playScale(currentScale.notes, true);
    }
  };

  const handleAnswer = (selectedScale: string) => {
    if (!currentScale || gameOver) return;

    const isCorrect = selectedScale === currentScale.name;
    
    saveResult({
      mode: 'scale',
      gameMode,
      correct: isCorrect,
      answer: currentScale.name,
      userAnswer: selectedScale,
      timestamp: Date.now(),
      difficulty,
    });
    ...
    if (gameMode === 'challenge') {
      ...
      if (isCorrect) {
        ...
        newState.score += 25 * (1 + newState.streak * 0.1);
      } else {
        newState.lives--;
        newState.streak = 0;
      }
  ...
  const availableScales = DIFFICULTY_PRESETS[difficulty].scales;
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <Card className="p-8">
        <div className="text-center space-y-6">
          ...
          {gameMode === 'zen' && showAnswer && currentScale && (
            <div className="flex items-center justify-center gap-2 p-4 bg-muted rounded-lg">
              <Eye className="w-5 h-5" />
              <span>答案: {currentScale.name}</span>
            </div>
          )}
